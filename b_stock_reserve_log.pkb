...
AS
  ...
  EVENT_ID_EXTEND_RESERVATION VARCHAR2(1) := '5';
  PROCEDURE logging (
    stock_id_in IN NUMBER,
    event_id_in IN VARCHAR2,
    event_date_in IN DATE
  ); 
  PROCEDURE force_cancel_logging (
    stock_id_in IN NUMBER,
    shop_id_in IN NUMBER,
...
AS
  ...
  BEGIN
    ...
    LOOP
      ...
      rec.reservation_deadline     := c1.reservation_deadline;
      rec.close_date               := c1.close_date;
      rec.shop_id                  := shop_id_in;
      rec.user_id                  := user_id_in;
      rec.close_flg                := c1.close_flg;
      rec.event_date               := event_date_in;
      IF c1.close_flg = 'N' THEN
        rec.event_id := EVENT_ID_CANCEL_RESERVATION;
      ELSE
        rec.event_id := EVENT_ID_CANCEL_CLOSE;
      END IF;

      INSERT INTO b_stocks_log values rec;
    END LOOP;
  END force_cancel_logging; 
